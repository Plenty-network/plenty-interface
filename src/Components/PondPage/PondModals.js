...
const PondModals = () => {
  ...
  const withDrawalFee = useSelector((state) => {
    ...
    return (
      state.ponds.pondsToRender?.find((x) => x.pondData.CONTRACT === modalData.contractAddress)
  ...
  return (
    <>
      <InfoTableModal
        type={modalData.open}
        open={
          modalData.open === POND_PAGE_MODAL.ROI || modalData.open === POND_PAGE_MODAL.WITHDRAWAL
        }
        onClose={onClose}
