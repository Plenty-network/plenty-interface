      ...
      <div className={clsx(styles.card, "p-4")}>
        ...
        <div>
          <div className={clsx("d-flex", styles.checkbox)}>
            <input value={checked[0]} type="checkbox" onChange={ev => setChecked([ev.target.checked, checked[1]])} id="d-01" />

            <label className={clsx(styles.disclaimerItem, "ml-3 mb-2 p-3")} htmlFor="d-01">
              I understand that I am using this product in beta at my own risk. Any losses incurred due to my actions are my own responsibility.
            </label>

          </div>

          <div className={clsx("d-flex", styles.checkbox)}>
            <input value={checked[1]} type="checkbox" onChange={ev => setChecked([checked[0], ev.target.checked])} id="d-02" />
        ...
        <Button onClick={onAccept} color={"primary"} disabled={!checked.every(x => x)} className="w-100">
          Confirm
        </Button>
      </div>
    </div>
