...
const computeTokenOutForRouteBaseV2 = (inputAmount, swapData, slippage) => {
  const initialData = {
...
// TOP
export const computeTokenOutForRouteBaseV3 = (input, allRoutes, slippage) => {
  try {
    const computeResponses = [];
    const bestRoute = {};
    allRoutes.forEach((route) => {
      const computedData = computeTokenOutForRouteBaseV2(
        input,
...
const computeTokenOutForRouteBaseByOutAmountV2 = (outputAmount, swapData, slippage) => {
  try {
...
// BOTTOM
export const computeTokenOutForRouteBaseByOutAmountV3 = (outputAmount, allRoutes, slippage) => {
  try {
    const computeResponses = [];
    const bestRoute = {};
    allRoutes.forEach((route) => {
      const computedData = computeTokenOutForRouteBaseByOutAmountV2(
        outputAmount,
        route.intermediateAMMData,
        slippage,
      );
