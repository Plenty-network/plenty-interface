...
export const getSwapDetails = (tokenIn,tokenOut) => {
    return dispatch => {
        ...
        swapApis.loadSwapData(tokenIn,tokenOut)
            .then(response => {
                dispatch(swapDetailsFetchSuccessfull(response));
            })
            .catch(swapDetailsError => {
                console.log({swapDetailsError});
                dispatch(swapDetailsFetchFailed());
            })
    }
}

export const setSwapTokenOutput = (tokenIn_amount, tokenIn_supply, tokenOut_supply, exchangeFee, slippage) => {
    return dispatch => {
        let tokenOutputDetails = swapApis.computeTokenOutput(tokenIn_amount, tokenIn_supply, tokenOut_supply, exchangeFee, slippage)
        dispatch ({
...
export const setEstimateOtherToken = (tokenIn_amount, tokenIn_supply, tokenOut_supply) => {
    return dispatch => {
        let estimateOtherToken = swapApis.estimateOtherToken(tokenIn_amount, tokenIn_supply, tokenOut_supply);
        dispatch({
...
export const computeRemoveTokens = (burnAmount, lpTotalSupply, tokenFirst_Supply, tokenSecond_Supply, slippage) => {
    let removeTokensOutput = swapApis.computeRemoveTokens(burnAmount, lpTotalSupply, tokenFirst_Supply, tokenSecond_Supply, slippage)
    return dispatch => {
        dispatch({
...
export const setLpTokenOutput = (tokenIn_amount, tokenOut_amount, tokenIn_supply, tokenOut_supply, lpTokenSupply) => {
    let lpOutput = swapApis.lpTokenOutput(tokenIn_amount, tokenOut_amount, tokenIn_supply, tokenOut_supply, lpTokenSupply)
    return dispatch => {
        dispatch({
...
export const swap = (tokenIn, tokenOut, minimumTokenOut, recipent, tokenInAmount, caller) => {
    return dispatch => {
        ...
        swapApis.swapTokens(tokenIn, tokenOut, minimumTokenOut, recipent, tokenInAmount, caller)
            .then(resp => {
...
export const removeLiquidity = (tokenA,tokenB,tokenA_MinimumRecieve,tokenB_MinimumRecieve,lpToken_Amount,caller) => {
    return dispatch => {
        dispatch(initiateRemoveLiquidity());
        swapApis.removeLiquidity(tokenA,tokenB,tokenA_MinimumRecieve,tokenB_MinimumRecieve,lpToken_Amount,caller)
            .then(resp => {
